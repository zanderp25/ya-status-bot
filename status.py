...
class Status(commands.Cog):
    ...
    async def notify(self, user):
        users = [
            (await self.bot.fetch_channel(config.notif_channel)).guild.get_member(user).mention
            for user in config.notif_users
        ]
        await (await self.bot.fetch_channel(config.notif_channel)).send(f'{", ".join(users)}: {user.name} is offline')
    ...
    async def status(self, ctx):
        ...
        await ctx.reply(
            embed=discord.Embed(
                ...
                color=discord.Color.red() if not user.status == discord.Status.online else discord.Color.green(),
            ).set_author(name=f"{user.name}", icon_url=f"{user.avatar_url}")
        )
    ...
    async def clear(self, ctx):
        if ctx.channel.id == config.channel:
            ...
            await ctx.reply(
                f"This command can only be used in {(await self.bot.fetch_channel(config.channel)).mention}"
            )
